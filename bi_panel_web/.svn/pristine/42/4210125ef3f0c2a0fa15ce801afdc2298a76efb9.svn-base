...
function preselect() {
    ...
    var counId = document.getElementById('counId');
    
    
    if(proId == null || proId.value == ''){
    	index = 0;
    	document.getElementById(s[0]).selectedIndex = index + 1;
    	change(1);
    }else if(proId.value.split('\\|')[0] == '-1'){
    	index = 0;
    	document.getElementById(s[0]).selectedIndex = index;
    	change(1);
    }else{
    	var indexArr = proId.value.split('\\|')[0].split('_');
    	index = parseInt(indexArr[indexArr.length - 1]);
    	document.getElementById(s[0]).selectedIndex = index + 1;
    	change(1);
    	
    	
    	if(!(cityId == null || cityId.value == '' || cityId.value.split('\\|')[0] == '-1')){
    		indexArr = cityId.value.split('\\|')[0].split('_');
        	index =  parseInt(indexArr[indexArr.length - 1]);
        	document.getElementById(s[1]).selectedIndex = index + 1;
        	change(2);
        	
        	if(!(counId == null || counId.value == '' || counId.value.split('\\|')[0] == '-1')){
        		indexArr = counId.value.split('\\|')[0].split('_');
            	index =  parseInt(indexArr[indexArr.length - 1]);
            	document.getElementById(s[2]).selectedIndex = index + 1;
        	}
        }
    }
}
...
function promptinfo()
{
    ...
    proId.value = s1.value+'|'+s1.options[s1.selectedIndex].text;
    cityId.value = s2.value+'|'+s2.options[s2.selectedIndex].text;
    counId.value = s3.value+'|'+s3.options[s3.selectedIndex].text;
}
