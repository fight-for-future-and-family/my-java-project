	...
	$.hourReport={
		...
		initEvent:function(){
			$(".tab-hd span").click(function(){
				$.hourReport.redict($(this).attr("view"));
			});
			...
			$.ajax({
				  ...
				  success: function(data){
					  ...
					  if(data.status == 'error'){
						  ...
						  $(".ajax_loading .ajax_loading_span").text("实时计算报表出错，邮件BI@hoolai.com!");
...
//					    		$("#newEquip_count").show();
//					    		$("#newEquipRate_count").show();
					    	}else if(ps.channel == 'client'){
//					    		$("#newEquip_count").hide();
//					    		$("#newEquipRate_count").hide();
...
//					    		$("#newEquipRate_count").show();
	                        ...
	                        $.hourReport.brokenDataDiv('installCount',textArr[0]);
...
//					    	$.hourReport.brokenDataDiv('newEquipCount',textArr[8]);
//					    	$.hourReport.brokenDataDiv('newEquipRate',Math.round((textArr[0]/textArr[8])*100*100)/100+'%');
					    	//Math.round(v.roleChoiceRate*100*100)/100 + '%'
					    	//Math.round((textArr[0]/textArr[8])*100*100)/100+'%'
					  }
				  }
				});
		},
		brokenTable_daily:function(reports,str){
			...
			var pay_count = 0,install_count = 0,dau_count=0,pu_count = 0,arpu = 0,arppu = 0,acu=0,pcu=0,avgUserTime=0,newEquip=0;
			...
			$.each(reports,function(i,v){
				...
				$("td.dnuArpu",trTemp).text(Math.round(v.installPayAmount/v.dnu/rate*100)/100);
			...
			return [install_count,dau_count,pay_count,arpu,arppu,pcu,acu,avgUserTime,newEquip];
		},
		...
		brokenTable_source:function(ps,str){
			...
			$('#dt_source_ajax button').click(function (){
		        ...
		        window.btnTimeout=setInterval(function(){
					$.gameUtil.btnTimeout($('#dt_source_ajax button'),window.btnTimeout);
				  },1000);
		        $("#downLoadForm").submit();
		    });
		...
		brokenTable_client1:function(reports,str){
            ...
            var pay_count = 0,install_count = 0,dau_count=0,pu_count = 0,arpu = 0,arppu = 0,acu=0,pcu=0,avgUserTime=0,newEquip;
		...
		change:function(exclass){
			...
			if(value == 'all'){
				...
				$.ajax({
					  ...
					  success: function(data){
					    var span = $("#s_c_span");
					    var str = '<select id="s_c" style="max-width:130px"><option value="-1">请选择...</option><option value="-99">所有渠道</option>';
				...
				$.ajax({
					  ...
					  success: function(data){
					    var span = $("#s_c_span");
					    var str = '<select id="s_c" style="max-width:130px"><option value="-1">请选择...</option><option value="-99">所有服务器</option>';
