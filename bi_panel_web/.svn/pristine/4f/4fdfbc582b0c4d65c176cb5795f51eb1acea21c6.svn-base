package com.hoolai.panel.web.manage.filter;
...
import com.hoolai.bi.report.util.HttpUtil;
...
public class ManageAuthenticationFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if(!isExclude && Constant.IS_MAINTAIN && !(Boolean)session.getAttribute("isAdmin")){
			httpResponse.sendRedirect(HttpUtil.getBasePath(httpRequest)+loginUrl);
			return;
		}else if (!isExclude&&(null == session.getAttribute(AbstractPanelController.SESSION_USER_KEY))) {
