...
<%
	String importMsg="";
	if(request.getSession().getAttribute("importDailyMsg")!=null){
		importMsg=request.getSession().getAttribute("importDailyMsg").toString();
	}
	request.getSession().setAttribute("importDailyMsg", "");
 %>
<head>
<meta charset="utf-8">
<title>游戏流水录入</title>
...
<script type="text/javascript" src="/js/gamesDailySave.js?v=${version}"></script>
			    	...
			    	<ul class="detail-ul">
			    		<li class="detail-li" style="height:45px;overflow: hidden;">
			    			<span style="width:30%;float: left;padding-left:5px;padding-top:10px;">
			    			    游戏系列名称：<input id="seriesName" class="b-radius5 p-input" style="margin-left:5px;width:180px"/>
			    			</span>
			    			<span style="width:20%;float: left;padding-left:5px;padding-top:10px;">
			    			    日期：<input id='ec_beginTime' class="b-radius5 p-input" style="margin-left:5px;width:110px"/>
			    			</span>
			    				...
			    				<!-- <button id="edit" class="btn btn-default btn-sm" style="width:100px;" onclick="edit()">修改</button> -->
			    				<!-- <button id="del" class="btn btn-default btn-sm" style="width:100px;" onclick="del()">删除</button> -->
...
<ul class="detail-ul">
<li class="detail-li" >
<div class="detail-left" style="width:50%">
<div style="float: left">
</div>
<form id="fileForm" action="/panel_bi/wanda/importCreativeRate.ac" method="post" enctype="multipart/form-data">
...
<%-- <font id="importMsg" color="red" style="margin-right: 20px;"><%=importMsg%></font> --%>
	...
	<table class="dataTable_edit" style="width: 100%;">
		...
		<tbody>
			<tr>
	    		...
	    		<td><input id="paymentAmount" type="text" class="b-radius5 p-input" /></td>
	    		<td><input id="remark" type="text" class="b-radius5 p-input" /></td>
