$(function(){
	...
	$.channelCps={
			init:function(){
				$.channelCps.initEvent();
				$.channelCps.queryData();
			},
			initEvent:function(){
...
//			},
			...
			queryData:function(){
	        	...
	        	//if(isl == 'false'){
	        		columnsDefArr.push({ 
	        			targets: ps.indicators != 'source' ? 6 : 5,
	        			render: function (a, b, v, d) {
	        				return v.entity.pu;
	        			}
	        		});
	        		columnsDefArr.push({ 
	        			targets: ps.indicators != 'source' ? 7 : 6,
	        			render: function (a, b, v, d) {
	        				return v.entity.paymentCnt;
	        			}
	        		});
	        		columnsDefArr.push({ 
	        			targets: ps.indicators != 'source' ? 8 : 7,
	        			render: function (a, b, v, d) {
	        				return Math.round(v.entity.paymentAmount/gameRate*100)/100;
	        			}
	        		});
	        	//}
			...
			checkParam:function(){
				var ps={
			      ...
			      if(ps.indicators != 'month'){
				      var dateArr = $.channelCps.processDate(ps.beginTime,ps.endTime);
