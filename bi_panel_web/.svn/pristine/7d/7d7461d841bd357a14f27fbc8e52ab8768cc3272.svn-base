$(function(){
	...
	$.newPayLevel={
			init:function(){
				$.newPayLevel.initEvent();
				$.newPayLevel.queryData();
			},
			initEvent:function(){
...
//			},
			...
			createTr:function(table,tbody,tempInterval,index,count_10_arr,count_all_arr,isPay){
				...
				for(var j=0;j<index;j++){
					...
					var point = '';
					if(isPay){
						point = $.newPayLevel.processCellData(Math.round(count_10_arr[j]/game.rate*100)/100,Math.round(count_all_arr[j]/game.rate*100)/100)
					}else{
						point = $.newPayLevel.processCellData(count_10_arr[j],count_all_arr[j])
					}
					td.html(point);
					tr.append(td);
			...
			queryData:function(){
				...
				$.ajax({
					  ...
					  success: function(data){
							...
							if(reports.length > 1){
								$.newPayLevel.getLevelDateValue(count_all_arr,reports[reports.length - 1]);
								...
								$.each(reports,function(i,v){
									if(v.level == -1){// 不出意外情况这是数组的最后一个元素
										$.newPayLevel.createTr(table,tbody,tempInterval,index,count_10_arr,count_all_arr,isPay);
										
										var tr = $("tbody tr",table).first().clone(true);
										$("td",tr).text(isPay ? '总消费额('+game.currency+')' :'总人数');
										for(var j=0;j<index;j++){
											var td = $("td",tr).first().clone(true);
											td.text(isPay ? Math.round(count_all_arr[j]/game.rate*100)/100 : count_all_arr[j]);
											tr.append(td);
							        	}
										$("td",tr).first().addClass("table-striped row-details-td");
										tbody.prepend(tr);
									}else if(v.level >= ((tempInterval-1) * 10 + 1) && v.level <= (tempInterval * 10)){
										$.newPayLevel.getLevelDateValue(count_10_arr,v);
									}else{
										$.newPayLevel.createTr(table,tbody,tempInterval,index,count_10_arr,count_all_arr,isPay);
						                    	   ...
						                    	   if(v.level >= ((data_index-1) * 10 + 1) && v.level <= (data_index * 10)){
						                    		  ...
						                    		  // td.removeClass();
						                    		   ...
						                    		   for(var i=1;i<=index;i++){
															...
															var value = $.newPayLevel.getItemValue(i,v);
														  	td.html($.newPayLevel.processCellData((isPay ? Math.round(value/game.rate*100)/100 : value),(isPay ? Math.round(count_all_arr[i-1]/game.rate*100)/100 : count_all_arr[i-1])));
														  	ctr.append(td);
			...
			checkParam:function(){
				...
				if(ps.indicators == null || ps.indicators == ''){
			    	  ps.indicators = 'all'; 
			    	   ...
			    	   if(ps.clientid != null && ps.clientid == '-1'){
			    		   return null;
			    	   }
			       }
			      ...
			      var dateArr = $.newPayLevel.processDate(ps.beginTime,ps.endTime);
			...
			change:function(){
				...
				if(value == 'all'){
					...
					$.ajax({
						  type: "POST",
						  url: "/panel_bi/payerLevel/getNewPayClient.ac",
						  data: $.param(ps),
						  dataType: "json",
						  success: function(data){
						    var span = $("#s_c_span");
						    var str = '<select id="s_c" name="s_c" style="max-width:240px"><option value="-1">请选择...</option>';
