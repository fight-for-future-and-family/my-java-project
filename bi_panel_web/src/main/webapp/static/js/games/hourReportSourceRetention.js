$(function(){
	...
	$(document).ready(function() {
	    $.hourReportSourceRetention.init();
	    $.timeZone.showTimeZone();
	});
	
	 var spendTimes = 0;
	$.hourReportSourceRetention={
		init:function(){
			$.hourReportSourceRetention.initEvent();
			$.hourReportSourceRetention.submit();
		},
		initEvent:function(){
			$(".detail-nav li").each(function(i,n){
				$(n).click(function(){
					$.hourReportSourceRetention.redict($(n).attr("id"));
				});
			});
			$("#channel").change(function(){
				$.hourReportSourceRetention.change($.hourReportSourceRetention);
			});
			$("#query").click(function(){
				$.hourReportSourceRetention.submit();
			});
			$("#redone").click(function(){
				$.hourReportSourceRetention.redone();
			});
		},
//		redict:function(view){
//			window.location.href='/panel_bi/hourReportSourceRetention/toHourReportSourceRetention.ac?id='+$("input[name='gamesId']").val()+'&view='+view;
//		},
		redict:function(view){
			var snid = $("input[name='snid']").val();
			var gameId = $("input[name='gameId']").val();
			window.location.href='/panel_bi/hourReportSourceRetention/toHourReportSourceRetention.ac?id='+$("input[name='gamesId']").val()+'&view='+view+'&snid='+snid+'&gameId='+gameId;
		},
		checkParam:function(){
			var ps={
		...
		redone:function(){
			var ps = $.hourReportSourceRetention.checkParam();
			$.ajax({
				type: "POST",
				url: "/panel_bi/hourReportSourceRetention/redoneRetentionHourReport.ac",
				data: $.param(ps),
				dataType: "json",
				success: function(data){
					if(data.status==0){
						...
						alert(data.msg);
						 ...
						 var viewHour = null;
						  var viewMinute = null;
						  viewHour = Number(data.hour);
				    	  viewMinute = Number(data.minute);
		...
		submit:function(){
			...
			if(ps.source == '-99'){
				if(ps.channel == 'source'){
					$.hourReportSourceRetention.brokenTable_source(ps);
				}
				return;
			}
			...
			$.ajax({
				  type: "POST",
				  url: "/panel_bi/hourReportSourceRetention/getHourReportSourceRetention.ac",
				  data: $.param(ps),
				  dataType: "json",
				  success: function(data){
					  ...
					  if(data.status == 'error'){
						  ...
						  var textArr = new Array(8);
					    	if(ps.channel == 'source'){
					    		$.hourReportSourceRetention.brokenTable_source1(data.sourceReports,str);
					    	}else{
					    		$.hourReportSourceRetention.brokenTable_daily(data.reports,str);
					    	}
					  }
				  }
				});
		...
		change:function(exclass){
			...
			if(value == 'all'){
				...
				$.ajax({
					  type: "POST",
					  url: "/panel_bi/hourReportSourceRetention/getGameSource.ac",
					  data: $.param(ps),
					  dataType: "json",
					  success: function(data){
					    var span = $("#s_c_span");
