					  ...
					  var reports = $.gameUtil.proReportData2(data.reports,'day',dateArr);
				    	...
				    	tab_reports = reports;
				    	 ...
				    	 if(indicators == 'all'){//总体(主要为基础数据)
							  ...
							  $("#life_bar_chart").hide();
							  $("#data4").hide();
							  $.gameView.brokenLineAndBar2(life_bar2,data.reports,dateArr);
		                      $.gameView.brokenTable2(data.reports);
			...
			function DrawEChart(ec){
				require(['theme/macarons'], function(tarTheme){
					...
					if ($("#queryType").val() == 'roleChoice') {
						...
						formatterStr1 += "<br/>{a2} : {c2} "+currency;
			...
			function DrawEChart(ec){
				require(['theme/macarons'], function(tarTheme){
					...
					$.each(dateArr[1],function(i,d){
						...
						$.each(ltvs,function(j,ltv){
							if(d == ltv.install_date){
								if(ltv.new_equip== null || ltv.new_equip == undefined
										|| ltv.new_equip == 0 || ltv.new_equip == '-'){
									return false;
								}
