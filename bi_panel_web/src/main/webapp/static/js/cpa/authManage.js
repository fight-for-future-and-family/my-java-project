$(function(){
	...
	$.authManage={
		init:function(){
			$.authManage.initEvent();
			$.authManage.queryData();
		},
		initEvent:function(){
...
//		},
		...
		queryData:function(){
		 var data = {
           ...
           $.ajax({
               "url":"/panel_bi/costPer/queryUserList.ac",
               "data":data,
               "type":"post",
               "error":function(){
                   alert("服务器未正常响应，请重试");
               ...
               "success":function(data){
            	   ...
            	   $('#dt_ajax_wrapper').remove();
	    			...
	    			$(".auth-btn",table).each(function(i,n){
						$(n).click(function(){
							$.authManage.sourceAuth($(n).attr("oid"));
						});
					});
		...
		sourceAuth:function(userid){
			...
			$.ajax({
	               "url":"/panel_bi/costPer/querySource.ac",
	               "type":"post",
	               "data":data,
	               "error":function(){
	                   alert("服务器未正常响应，请重试");
	               ...
	               "success":function(data){
	            	   ...
	            	   $('#dt_had_wrapper').remove();
	       			   ...
	       			   var had_jq_table = $.authManage.createTable(data.hadDimensions,hadTable,'dt_had');
	       			   ...
	       			   var nothad_jq_table = $.authManage.createTable(data.notHadDimensions,needTable,'dt_nothad');
	    			...
	    			$(".theme-popover .save-btn").click(function(){
	    				...
	    				$.ajax({
	    		               "url":"/panel_bi/costPer/authManager.ac",
	    		               "data":$.param(ps_data),
	    		               "type":"post",
	    		               "error":function(){
	    		                   alert("服务器未正常响应，请重试");
