$(function(){
	...
	$.installLevel={
			init:function(){
				$.installLevel.initEvent();
				$.installLevel.queryData();
			},
			initEvent:function(){
...
//			},
			...
			createTr:function(table,tbody,tempInterval,index,count_10_arr,count_all_arr){
				...
				for(var j=0;j<index;j++){
					...
					var point = $.installLevel.processCellData(count_10_arr[j],count_all_arr[j])
					td.html(point);
					tr.append(td);
			...
			queryData:function(){
				var ps = $.installLevel.checkParam();
				if(ps == null){
					return;
			    }
				...
				$.ajax({
					  ...
					  success: function(data){
							...
							if(reports.length > 1){
								$.installLevel.getLevelDateValue(count_all_arr,reports[reports.length - 1]);
								tempInterval = Math.ceil(reports[0].level / 10);
								
								$.each(reports,function(i,v){
									if(v.level == -1){//不出意外情况这是数组的最后一个元素
										$.installLevel.createTr(table,tbody,tempInterval,index,count_10_arr,count_all_arr);
										...
										$.installLevel.getLevelDateValue(count_10_arr,v);
									}else{
										$.installLevel.createTr(table,tbody,tempInterval,index,count_10_arr,count_all_arr);
						                   ...
						                   if (jq_table.fnIsOpen(nTr)) //判断是否已打开
						                   {
						                       ...
						                       $.each(reports,function(i,v){
						                    	   if(v.level >= ((data_index-1) * 10 + 1) && v.level <= (data_index * 10)){
						                    		   ...
						                    		   for(var i=0;i<index;i++){
															var td = $("td",tr).eq(0).clone(true);
														  	td.html($.installLevel.processCellData($.installLevel.getItemValue(i,v),count_all_arr[i]));
														  	ctr.append(td);
