$(function(){
	...
	$.hourLtvReport={
		init:function(){
			$.hourLtvReport.initEvent();
			$.hourLtvReport.submit();
		},
		initEvent:function(){
			$(".tab-hd span").click(function(){
				$.hourLtvReport.redict($(this).attr("view"));
			});
			
			$(".detail-nav li").each(function(i,n){
				$(n).click(function(){
					$.hourLtvReport.redict($(n).attr("id"));
				});
			});
			$("#channel").change(function(){
				$.hourLtvReport.change($.hourLtvReport);
			});
			$("#query").click(function(){
				$.hourLtvReport.submit();
			});
			$("#redone").click(function(){
				$.hourLtvReport.redone();
			});
		},
...
//		},
		...
		redone:function(){
			var ps = $.hourLtvReport.checkParam();
			$.ajax({
				type: "POST",
		...
		submit:function(){
			...
			if(ps.source == '-99'){
				if(ps.channel == 'source'){
					$.hourLtvReport.brokenTable_source(ps);
				}
				return;
			}
				  ...
				  success: function(data){
					  ...
					  if(data.status == 'error'){
						  ...
						  var textArr = new Array(8);
					    	if(ps.channel == 'source'){
					    		$.hourLtvReport.brokenTable_source1(data.sourceReports,str);
					    	}else{
					    		$.hourLtvReport.brokenTable_daily(data.reports,str);
					    	}
					  }
				  }
				});
