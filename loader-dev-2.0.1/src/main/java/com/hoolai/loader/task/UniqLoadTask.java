		...
		try {
			...
			if (localFile.exists()) {// 去重后的文件存在
				...
				if(StringUtil.isRevision(path)){
					...
					oldFilePath = "/user/hive/warehouse/nt_" + param[0] +"/snid=" + param[1] + "/gameid=" + param[2] + "/ds=" + param[3] + "/" + localFile.getName() + Constant.lzoSuffix;
				} else {
					...
					oldFilePath = tmpHdfs.substring(0, tmpHdfs.indexOf(".uniq")) + Constant.lzoSuffix;
				}
